...
export default function Home() {
  ...
  const warframes = Array.from(items).filter((item) => item.isPrime!==true && item.productCategory==="Suits");
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      {warframes.map((warframe) => (
        <div key={warframe.name} className="flex flex-col items-center">
          ...
          <Image src={warframe.wikiaThumbnail} alt="Warframe" width={200} height={200}/>
